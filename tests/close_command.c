...
int main(){
  ...
  if(sendMessage(socket, REQ_OK | CLOSE, "") != MSG_OK) ASSERT(0, "Send close command failed");

  if(recvMessage(socket, &msg) != MSG_OK) ASSERT(0, "Receive close command message failed");

  debugMessage(&msg);
  ASSERT(hasFlag(msg.status, RES_OK | CLOSE), "Connection closed successfully");
